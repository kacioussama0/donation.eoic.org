                    ...
                    <div class="card-body vstack gap-3">
                        ...
                        <div class="progress" role="progressbar" aria-label="Animated striped example"
                            ...
                            <div class="progress-bar progress-bar-striped progress-bar-animated"
                                 style="width: {{($project->orders->where('status', 'paid')->sum('total_price') * 100) / $project->price }}%">{{ round(($project->orders->where('status', 'paid')->sum('total_price') * 100) / $project->price,2) > 100 ? 100 : round(($project->orders->where('status', 'paid')->sum('total_price') * 100) / $project->price,2) }}
                                %
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card shadow border-0 rounded-4">
                            <div class="card-body vstack gap-3">
                                ...
                                @else
                                    ...
                                    <form action="{{url('/checkout')}}" class="d-flex flex-column-reverse"
                                          method="POST">

                                        <div class="btn-group suggestions mt-3" role="group"
                                             aria-label="Basic radio toggle button group">

                                            @for($i = 1 ; $i <= 12 ; $i+=2)
                                                <input type="radio" class="btn-check" name="btnradio"
                                                       id="btnradio{{$i}}" autocomplete="off"
                                                       max="{{$project->price}}  value="{{$project->price_one * $i}}">
                                        ...
                                        <div class="d-flex align-items-center ">
                                            ...
                                            <input type="hidden" name="image"
                                                   value="{{asset('storage/' . $project->thumbnail)}}">
                                            <button type="submit"
                ...
                <div class="col-12 text-center">
                    ...
                    <div class="card shadow border-0 rounded-4">
                        <div class="card-body">
                            ...
                            <a href="https://www.facebook.com/sharer.php?u={{url('/projects/' . $project->slug())}}"
                               class="text-decoration-none">
                                <i class="fa-brands fa-facebook fa-2x me-3"></i>
                            </a>

                            <a href="https://twitter.com/intent/tweet?text={{$project->title()}}&url={{url('/projects/' . $project->slug())}}"
                               class="text-decoration-none">
